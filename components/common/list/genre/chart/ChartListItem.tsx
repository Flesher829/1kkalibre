import { Album, ChartsData } from "@/lib/@types";
...
export const ChartListItem: FunctionComponent<ChartsData> = ({ data }) => {
  console.log(data);
  return (
    <>
      <Container>
        <Wrapper>
          <AlbumCoverContainer>
            <div className='album__cover'>
              <img
                src={data?.albums?.data[0].cover_medium}
                alt={data?.albums?.data[0].title}
              />
            </div>
            <div className='album__text'>
              <span>Top 10 Albums</span>
        ...
        <div className='content'>
          ...
          {data?.albums?.data.map((alb: Album, index: number) => (
